...
def get_similarties(document_vectors, query_vectors):
    return cosine_similarity(document_vectors, query_vectors)


def get_document_vectors(bert_embedding, documents):
    embeddings = bert_embedding(documents)

    document_vectors = []

    for embedding in embeddings:
        document_vector = np.mean(embedding[1], axis=0)
        document_vectors.append(document_vector)

    return document_vectors


def get_query_vectors(bert_embedding, queries):
    embeddings = bert_embedding(queries)

    query_vectors = []

    for embedding in embeddings:
        query_vector = np.mean(embedding[1], axis=0)
...
def process_documents(document_file_name):
    ...
    document_str_list = remove_url(document_file.read()).split("\n")
...
def process_query(query_str):
    ...
    text = remove_tag(query_list[2], 'title')
...
#     model='bert_24_1024_16', dataset_name='book_corpus_wiki_en_cased')
# document_vectors = get_document_vectors(bert_embedding, document_set['text'])
# query_vectors = get_query_vectors(bert_embedding, query_set['text'])
